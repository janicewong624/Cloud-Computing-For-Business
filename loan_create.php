...
$selectedDate = $_GET['loan_date'] ?? date('Y-m-d');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($loan_date === '' || $time_slot === '' || $quantity < 1 || $purpose === '') {
        ...
        if (!$equipmentRow) {
            ...
            $stmt = $conn->prepare('SELECT COALESCE(SUM(quantity), 0) AS loaned FROM equipment_loans WHERE equipment_id = ? AND loan_date = ? AND time_slot = ?');
            $stmt->bind_param('iss', $equipment_id, $loan_date, $time_slot);
            $stmt->execute();
            ...
            if ($loaned + $quantity > $equipmentRow['total_units']) {
                ...
                $stmt = $conn->prepare('INSERT INTO equipment_loans (user_id, equipment_id, loan_date, time_slot, quantity, purpose) VALUES (?, ?, ?, ?, ?, ?)');
                $stmt->bind_param('iissis', $uid, $equipment_id, $loan_date, $time_slot, $quantity, $purpose);
                $stmt->execute();
                $stmt->close();
                $conn->commit();
...
<option value="<?= (int)$e['id'] ?>" <?= $e['id'] == $selectedEquipment ? 'selected' : '' ?>><?= htmlspecialchars($e['equipment_name']) ?> (<?= (int)$e['total_units'] ?> units)</option>
<?php endwhile; ?>
</select>
</label>
<label>Loan Date <input type="date" name="loan_date" id="loan-date" value="<?= htmlspecialchars($selectedDate) ?>" min="<?= date('Y-m-d') ?>" required></label>
<label>Time Slot
...
<label>Purpose <input type="text" name="purpose" placeholder="e.g. Group presentation rehearsal" required></label>
<button type="submit">Loan Now</button>
</form>
<script>
(function () {
    ...
    function refreshSlots() {
        ...
        fetch('equipment_availability.php?equipment_id=' + encodeURIComponent(equipmentId) + '&loan_date=' + encodeURIComponent(date))
