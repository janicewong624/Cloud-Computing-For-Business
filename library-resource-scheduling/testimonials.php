...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($room_id < 1 || $comment === '' || $rating < 1 || $rating > 5) {
        $error = 'Please choose a room, write a comment, and pick a rating between 1 and 5.';
    } else {
        $stmt = $conn->prepare('INSERT INTO testimonials (user_id, room_id, comment, rating) VALUES (?, ?, ?, ?)');
        $stmt->bind_param('iisi', $uid, $room_id, $comment, $rating);
        if ($stmt->execute()) {
            $stmt->close();
...
$rooms = $conn->query('SELECT id, room_name FROM rooms ORDER BY room_name')->fetch_all(MYSQLI_ASSOC);

$testimonials = $conn->query('
    SELECT t.id, t.comment, t.rating, t.created_at, t.user_id, u.name AS user_name, r.room_name
    FROM testimonials t
...
<h1>Testimonials</h1>
<p>What students are saying about booking discussion rooms.</p>
</div>
...
<option value="<?= (int)$r['id'] ?>"><?= htmlspecialchars($r['room_name']) ?></option>
<?php endforeach; ?>
</select>
</label>
<label>Rating
